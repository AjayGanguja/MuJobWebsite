...
namespace JobWebSite.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult EditPosfile()
        {
            var UserID = User.Identity.GetUserId();
            var user = db.Users.Where(a => a.Id == UserID).SingleOrDefault();
            EditViewModel Profile = new EditViewModel(user);
            ViewBag.UserType = new SelectList(db.Roles.Where(a => !a.Name.Contains("Admin")).ToList(), "Name", "Name",Profile.UserType);
            return View(Profile);
        }
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> EditPosfile(EditViewModel model)
        {
            var UserId = User.Identity.GetUserId();
            ...
            {
                ViewBag.Message = "كلمة السر ليست صحيحة";
            }
            else
            {
                if (ModelState.IsValid)
                {
                    var newPasswordHashed = UserManager.PasswordHasher.HashPassword(model.NewPassword);
